...
const PercentageCalculator = () => {
  ...
  const calculate1 = () => {
    ...
    try {
      ...
      updateAIInsight(
        { calculationType: "Percentage of Number", percent: percentValue, number: numberValue },
        { result: finalResult, explanation: `${percentValue}% of ${numberValue} = ${finalResult}` }
      );
    } catch (error) {
  ...
  const calculate2 = () => {
    ...
    if (!isNaN(part) && !isNaN(total) && total !== 0) {
      const result = parseFloat(((part / total) * 100).toFixed(2));
      setResult2(result);

      updateAIInsight(
        { calculationType: "What Percentage", part, total },
        { result: result + "%", explanation: `${part} is ${result}% of ${total}` }
      );
    }
  };
  ...
  const calculate3 = () => {
    ...
    if (!isNaN(oldVal) && !isNaN(newVal) && oldVal !== 0) {
      const result = parseFloat((((newVal - oldVal) / oldVal) * 100).toFixed(2));
      setResult3(result);

      updateAIInsight(
        { calculationType: "Percentage Change", originalValue: oldVal, newValue: newVal },
        { 
          result: result + "%", 
          changeType: result > 0 ? "Increase" : result < 0 ? "Decrease" : "No Change",
      ...
      >
        <Tabs defaultValue="tab1" className="w-full">
          ...
          <TabsContent value="tab2" className="space-y-6 mt-6">
            ...
            <Button onClick={() => handleCalculation(calculate2)} className="w-full" size="lg" disabled={isCalculating}>
              {isCalculating ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Calculating...</> : "Calculate"}
            </Button>
            {result2 !== null && (
              <div className="p-6 bg-primary/10 rounded-lg text-center animate-fade-in">
          ...
          <TabsContent value="tab3" className="space-y-6 mt-6">
            ...
            <Button onClick={() => handleCalculation(calculate3)} className="w-full" size="lg" disabled={isCalculating}>
              {isCalculating ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Calculating...</> : "Calculate"}
            </Button>
            {result3 !== null && (
              <div className="p-6 bg-primary/10 rounded-lg text-center animate-fade-in">
